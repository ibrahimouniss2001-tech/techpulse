...
def generar_contenido(tema: str, categoria: str, url_afiliado: str | None = None) -> dict:
    ...
    articulo_response = client.messages.create(
        model="claude-sonnet-4-6",
        max_tokens=2048,
        system=ARTICLE_SYSTEM,
        messages=[{"role": "user", "content": articulo_prompt}],
    )
    raw_article = articulo_response.content[0].text.strip()
    ...
    # 2. Guión TikTok
    guion_response = client.messages.create(
        model="claude-haiku-4-5-20251001",
        max_tokens=512,
        system=TIKTOK_SYSTEM,
        messages=[{"role": "user", "content": f"Tema del vídeo: {tema}"}],
    )
    texto_guion = guion_response.content[0].text.strip()
...
def extraer_temas_de_texto(texto: str) -> list[dict]:
    ...
    """
    # Limita el texto para no exceder el contexto
    ...
    response = client.messages.create(
        ...
        messages=[{
            "role": "user",
            "content": f"Extrae los temas de artículos de este documento:\n\n{texto_recortado}"
        }],
    )

    raw = response.content[0].text.strip()

    # Extrae el array JSON aunque haya texto alrededor
